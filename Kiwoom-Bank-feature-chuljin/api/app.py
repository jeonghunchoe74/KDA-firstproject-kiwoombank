...
async def analyze_many(payload: Dict[str, Any]):
    # 1. 'companies' 리스트를 추출합니다. (["삼성전자", "키움증권", ...])
    companies = payload.get("companies", [])
    if not companies:
        raise HTTPException(status_code=400, detail="companies list required")

    results = []
    # 2. [핵심] 리스트에 있는 회사 이름을 하나씩 'for' 루프로 반복합니다.
    for name in companies:
        try:
            # 3. 각 회사에 대해 'predict_for_company' 함수를 호출합니다.
            #    (run_in_threadpool은 무거운 작업이 서버를 막지 않게 해줍니다)
            res = await run_in_threadpool(predict_for_company, name)
            res["company"] = name
            res["updated_at"] = datetime.utcnow().isoformat() + "Z"
            
            # 4. 성공하면 결과 리스트에 추가합니다.
            results.append(res)
        except Exception as e:
            # 5. 실패하면 (e.g. CSV 파일 없음) 에러를 리스트에 추가합니다.
            results.append({"company": name, "error": str(e)})
    
    # 6. 모든 결과를 모아서 프론트엔드에 JSON으로 응답합니다.
