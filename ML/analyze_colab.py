...
NEWS_PER_PAGE = 30    # Colab 속도 고려: 기사 30개
MAX_PAGES = 1
REQUEST_TIMEOUT = 10
...
def weighted_aggregate(results, weights):
    ...
    w = np.array(weights if weights else [1.0]*len(results))
...
def translate_openai(texts, client):
    ...
    for t in texts:
        if t in translation_cache:
            out.append(translation_cache[t])
            continue
        try:
...
def init_finbert_pipeline():
    ...
    return pipeline("text-classification", model=FINBERT_MODEL, top_k=None, device=device)
...
def finbert_scores(texts, finbert):
    ...
    for scores in outputs:
        ...
        for s in scores:
            ...
            if lab: d[lab] = float(s["score"])
        results.append(d)
    return results

# ============ 회사별 뉴스 피처 계산 ============
def analyze_company(name, client, finbert):
...
def main():
    ...
    for name in companies:
        print(f"\n=== {name} 분석 시작 ===")
        feats = analyze_company(name, client, finbert)
